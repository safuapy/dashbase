...
static int AppInitRawTx(int argc, char* argv[])
{
    ...
    if (argc < 2 || HelpRequested(gArgs)) {
        ...
        std::string strUsage = PACKAGE_NAME " dashbase-tx utility version " + FormatFullVersion() + "\n\n" +
            "Usage:  dashbase-tx [options] <hex-tx> [commands]  Update hex-encoded dashbase transaction\n" +
            "or:     dashbase-tx [options] -create [commands]   Create hex-encoded dashbase transaction\n" +
            "\n";
...
static int CommandLineRawTx(int argc, char* argv[])
{
    ...
    try {
        ...
        if (!fCreateBlank) {
            ...
            // param: hex-encoded dashbase transaction
