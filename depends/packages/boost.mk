...
$(package)_download_path=https://archives.boost.org/release/$(subst _,.,$($(package)_version))/source/
...
$(package)_cxxflags=-std=c++14 -fvisibility=hidden -Wno-enum-constexpr-conversion
...
define $(package)_preprocess_cmds
  ...
  echo "using $($(package)_toolset_$(host_os)) : : $($(package)_cxx) : <cflags>\"$($(package)_cflags)\" <cxxflags>\"$($(package)_cxxflags) $($(package)_cxxflags_$(host_os))\" <compileflags>\"$($(package)_cppflags)\" <linkflags>\"$($(package)_ldflags)\" <archiver>\"$($(package)_ar)\" <striper>\"$(host_STRIP)\"  <ranlib>\"$(host_RANLIB)\" <rc>\"$(host_WINDRES)\" : ;" > user-config.jam
endef
